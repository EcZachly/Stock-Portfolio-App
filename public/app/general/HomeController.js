...
HomePageController.controller("HomePageController", ["$scope", "$location", "$rootScope", "$sessionStorage", "StockService", function($scope, $location, $rootScope, $sessionStorage, StockService,  $nvd3) {
    ...
    StockService.getSymbols().then(function(data){
        $scope.symbols = data.map(function(v){
            return v.toUpperCase();
        })
    })

    $scope.chosenSymbol = 'ACAD';
    $scope.onChange = function(){
        updateQuotes($scope.chosenSymbol.toLowerCase());
    };
    function updateQuotes(symbol) {
        StockService.getQuotes(symbol).then(function (data) {
            console.log(data);
            $scope.chart_data = data;
            $scope.chart_options = null;
            setOptions();
        })
    }
    $scope.onChange();
